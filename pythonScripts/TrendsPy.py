...
path = r"C:\Users\rodgersja\Documents\Google PyTrends"
...
keywordcsv = r"C:\Users\rodgersja\Documents\Google PyTrends\keywords.csv"
...
for index, row in keywords.iterrows():
    ...
    connector.build_payload(payload)
    ...
    trenddata['date'] = trenddata['date'].str.extract(r'(^[0-9]{4}\-[0-9]{2}\-[0-9]{2}) \-.*')
    trenddata = trenddata.dropna()
    trenddata['date'] = pd.to_datetime(trenddata['date'])
    trenddata['year'] = pd.DatetimeIndex(trenddata['date'])
    trenddata['month'] = pd.DatetimeIndex(trenddata['date'])
    trenddata['day'] = pd.DatetimeIndex(trenddata['date'])
